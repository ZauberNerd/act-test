...
(async () => {
  const containerName = (0, import_core.getState)("container-name");
  if (!containerName) {
    return;
  }
  const dockerBin = await (0, import_io.which)("docker", true);
  try {
    (0, import_core.startGroup)("Container");
    await (0, import_exec.exec)(dockerBin, ["logs", containerName]);
  } catch (err) {
  ...
  try {
    await (0, import_exec.exec)(dockerBin, ["kill", containerName]);
  } catch (err) {
    (0, import_core.warning)(`Failed to kill container '${containerName}': ${err}`);
  }
  try {
    await (0, import_exec.exec)(dockerBin, ["rm", containerName]);
  } catch (err) {
    (0, import_core.warning)(`Failed to remove container '${containerName}': ${err}`);
  }
})();
