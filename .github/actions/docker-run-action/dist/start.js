...
(async () => {
  const dockerfile = (0, import_core.getInput)("dockerfile", {
    required: false,
    trimWhitespace: true
  });
  const image = (0, import_core.getInput)("image", {
    required: true,
    trimWhitespace: true
  });
  const args = (0, import_core.getMultilineInput)("args", { trimWhitespace: true });
  const command = (0, import_core.getInput)("command");
  const detach = (0, import_core.getInput)("detach", {
    required: true
  }) === "true";
  const pull = (0, import_core.getInput)("pull", {
    required: true
  });
  const containerName = (0, import_node_crypto.randomUUID)();
  const dockerBin = await (0, import_io.which)("docker", true);
  if (dockerfile) {
    await (0, import_exec.exec)(
      dockerBin,
      ["build", "--rm", "--tag", image, "--file", (0, import_path.basename)(dockerfile), "."],
      {
        cwd: (0, import_path.dirname)(dockerfile)
      }
    );
  }
  const dockerArgs = [
  ...
  await (0, import_exec.exec)(dockerBin, [...dockerArgs, image, ...dockerCommands]);
  (0, import_core.saveState)("container-name", containerName);
})();
