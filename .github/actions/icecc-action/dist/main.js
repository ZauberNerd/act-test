...
async function bash(command) {
  return await (0, import_exec.exec)("/bin/bash", ["-c", command]);
}
async function main() {
  ...
  await bash(`sudo sed -i 's/ICECC_SCHEDULER_HOST=""/ICECC_SCHEDULER_HOST="${schedulerHost}"/' /etc/icecc/icecc.conf`);
  core.addPath("/usr/lib/icecc/bin");
  if (isScheduler) {
    await bash("sudo systemctl start icecc-scheduler.service");
    const readyServer = (0, import_node_child_process.spawn)(`node`, ["-e", 'http.createServer((_, res) => res.end("ready")).listen(47249)'], { detached: true, stdio: "ignore" });
    readyServer.unref();
    core.info(`Started ready server for scheduler with pid "${readyServer.pid}"`);
